...
# FIXED: changed "species" to "species_id
```
...
bird_totals_by_park <- birds_df %>% 
  left_join(sites_1) %>% 
# implied that they join by site_id
  group_by(park_name) %>% 
...
# semi join only keeps obs in x if they appear in y and no col are added to x, so changed semi_join() to left_join() so parks_name() is retained
```
...
selected_parks <- common_birds %>% 
  filter(park_name %in% c("Cholla Cove", 
                          "Desert Willow", 
                          "Nueve", 
                          "Orme", 
                          "Palomino", 
...
# FIXED: added aes() to the first line
# also the \n in the total bird counts makes it return one line down
```

The code below exports the graph as a .jpg and saves in the project root.
```{r}
ggsave(here("figs", "bird_counts.jpg"), height = 6, width = 12)
